...
function ArticlesControl() {
  ...
  async function handleBtnSubmition(e) {
    ...
    const formData = {
      name: document.querySelector("#productNameForm").value,
      price: document.querySelector("#productPriceForm").value,
      imgs: i
    };
    console.log("Form data", formData)
    console.log(chosenToUpd)
    console.log(chosenToUpd)
    await axios({url: `http://localhost:5001/products?id=${chosenToUpd.id}`, method: "PUT", data: { item: { ...formData} } })
